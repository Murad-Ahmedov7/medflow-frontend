...
export function WalletSuccessPage() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      // Backend retries 4× with 1.5 s delay internally, so this single call
      // already handles the race where Stripe hasn't marked the session "paid" yet.
      // Idempotent: safe to call even if webhook already ran.
      try {
        const res = await axiosInstance.post<ApiResult<SessionResult>>(
          API_ENDPOINTS.wallet.fulfillSession(sessionId!),
        );
      ...
      // Step 2 — poll session endpoint up to 10 × 1.5 s.
      // This path is only hit when the fulfill-session call failed with a network error.
      for (let i = 0; i < 10; i++) {
        if (cancelled) return;
        await new Promise(r => setTimeout(r, 1500));
        try {
          const res = await axiosInstance.get<ApiResult<SessionResult>>(
            API_ENDPOINTS.wallet.session(sessionId!),
          );
  ...
  // Auto-navigate to /wallet after 5 s once data is loaded
  useEffect(() => {
    if (!loading) {
    ...
    return (
      <div className="min-h-[70vh] flex items-center justify-center px-4">
        <div className="w-full max-w-sm bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-xl p-8 flex flex-col items-center text-center gap-5">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">{t('wallet.success.verifying')}</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1.5 leading-relaxed">
              {t('wallet.success.verifyingDesc')}
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-[70vh] flex items-center justify-center px-4">
      <PaymentSuccessModal
        ...
        message={t('wallet.success.title')}
        description={t('wallet.success.desc')}
        amount={session?.paidAmount ?? undefined}
        amountLabel={t('wallet.success.amountAdded')}
        balance={session?.currentBalance ?? 0}
        balanceLabel={t('wallet.success.newBalance')}
        ctaLabel={
          <span className="flex items-center gap-2">
            {t('wallet.success.goToWallet')}
            <ArrowRight size={14} />
          </span>
        }
