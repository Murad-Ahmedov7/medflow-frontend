...
export function SignInPage() {
  ...
  return (
    <AuthLayout title={t('auth.doctorPortal')} subtitle={t('auth.welcomeBack')}>
      <form onSubmit={handleSubmit((data) => signIn(data))} className="space-y-5">
        ...
        <Input
          ...
          rightIcon={
            <button type="button" onClick={() => setShowPassword((v) => !v)} className="text-slate-400 hover:text-slate-600 transition-colors">
              {showPassword ? <EyeOff size={16} /> : <Eye size={16} />}
            </button>
          }
          error={errors.password?.message}
          autoComplete="current-password"
          {...register('password', { required: t('auth.validation.passwordRequired'), minLength: { value: 8, message: t('auth.validation.passwordMin') } })}
        />
