...
interface PaymentSuccessModalProps {
  open: boolean;
  /** Headline shown under the checkmark, e.g. "Wallet funded successfully." */
  message: string;
  /** Optional supporting line under the headline */
  description?: string;
  /** Amount line shown in an emerald highlight row — omit if not applicable */
  amount?: number;
  amountLabel?: string;
  /** 'credit' shows "+$X" (e.g. top-up), 'debit' shows "-$X" (e.g. a purchase charge). Defaults to 'credit'. */
  amountSign?: 'credit' | 'debit';
  /** New balance line — omit if not applicable (e.g. a stock purchase with no patient wallet involved) */
  balance?: number;
  balanceLabel?: string;
  /** Primary CTA content — omit to render no button (e.g. auto-dismissed modal) */
  ctaLabel?: React.ReactNode;
  onCta?: () => void;
  onClose: () => void;
}

// Shared visual identity for every payment-success moment in the app (wallet top-up,
// prescription purchase, etc.) — same spring-in checkmark, same card, same colors,
// so every successful transaction feels like the same product moment.
                  ...
                  {amount != null && (
                    <div className="flex items-center justify-between px-4 py-3 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-900/40">
                      ...
                      <span className="text-base font-bold text-emerald-700 dark:text-emerald-400 tabular-nums">
                        {amountSign === 'debit' ? '-' : '+'}${amount.toFixed(2)}
                      </span>
                    </div>
                  )}
