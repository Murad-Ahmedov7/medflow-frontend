...
import { MessageCircle, Send, ChevronLeft, ChevronDown, Users, Clock, Lock, CheckCheck, Ban } from 'lucide-react';
...
function MessageTicks({ msg }: { msg: MessageDto }) {
...
function groupChatsByParticipant(chats: ChatSummaryDto[], myUserId: string): DoctorGroup[] {
  ...
  for (const chat of chats) {
    ...
    if (!group) {
      group = { patientId: key, patientName: other.fullName, patientAvatarUrl: other.avatarUrl, totalUnread: 0, chats: [] };
  ...
  return Array.from(groups.values()).sort((a, b) => b.totalUnread - a.totalUnread || a.patientName.localeCompare(b.patientName));
}

function ParticipantGroupCard({ group, activeChatId, onSelectChat }: {
  group: DoctorGroup; activeChatId: string | undefined; onSelectChat: (id: string) => void;
}) {
    ...
    <div className="rounded-xl overflow-hidden">
      ...
      >
        <div className="relative shrink-0">
          <UserAvatar url={group.patientAvatarUrl} name={group.patientName} size={10} />
          {group.totalUnread > 0 && (
        ...
        <div className="flex-1 min-w-0 flex items-center gap-1.5">
          <span className="font-medium text-sm text-slate-800 dark:text-slate-100 truncate">{group.patientName}</span>
...
export function ChatPage() {
  ...
  const doctorGroups = useMemo(() => groupChatsByParticipant(chats, myUserId), [chats, myUserId]);
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4 text-center px-4">
        <div className="w-16 h-16 bg-cyan-50 dark:bg-cyan-900/20 rounded-full flex items-center justify-center">
          <Lock className="w-8 h-8 text-cyan-500" />
        </div>
        <h2 className="text-xl font-semibold text-slate-800 dark:text-slate-100">
          {t('chat.signInRequired', 'Sign in to access messages')}
        </h2>
        <p className="text-sm text-slate-500 dark:text-slate-400 max-w-sm">
          {t('chat.signInRequiredDesc', 'You need to be signed in to view and send messages.')}
        </p>
        <Link
          to="/sign-in"
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
      <div className="flex h-[calc(100vh-12rem)] bg-white dark:bg-slate-900 rounded-2xl overflow-hidden border border-slate-100 dark:border-slate-800 shadow-sm">
        {/* Sidebar */}
        <div className={cn(
          'flex flex-col border-r border-slate-100 dark:border-slate-800',
          activeChatId ? 'hidden md:flex w-72' : 'flex w-full md:w-72',
        )}>
          ...
          <div className="flex-1 overflow-y-auto py-2 px-2 space-y-1">
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center h-full py-12 text-slate-400">
                ...
                <p className="text-sm text-center">{t('chat.noConversations', 'No conversations yet')}</p>
                <p className="text-xs text-center mt-1 text-slate-400">{t('chat.bookAppointmentHint', 'Book an appointment to chat with your doctor')}</p>
              </div>
            ) : (
        ...
        <AnimatePresence mode="wait">
          {activeChatId && activeChat ? (
            <motion.div key={activeChatId} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.2 }} className="flex-1 flex flex-col">
              <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-100 dark:border-slate-800">
                <button onClick={() => setActiveChatId(undefined)} className="md:hidden p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800">
              ...
              <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
                {messages.map(msg => {
                  ...
                  return (
                    <motion.div key={msg.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className={cn('flex', isMine ? 'justify-end' : 'justify-start')}>
                      <div className="max-w-[70%]">
                        {!isMine && <p className="text-[11px] text-slate-400 mb-1 ml-1">{msg.senderName}</p>}
                        <div className={cn('px-3 py-2 rounded-2xl text-sm', isMine ? 'bg-cyan-500 text-white rounded-tr-sm' : 'bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-100 rounded-tl-sm')}>
                          {msg.body}
                        </div>
                        <p className={cn('text-[10px] text-slate-400 mt-1 flex items-center gap-0.5', isMine ? 'justify-end' : 'justify-start')}>
                          <Clock className="w-2.5 h-2.5 shrink-0" />{formatTime(msg.sentAt)}
              ...
              {isChatReadOnly(activeChat) ? (
                ...
                <div className="px-4 py-3 border-t border-slate-100 dark:border-slate-800 flex items-center gap-2">
                  ...
                  />
                  <button onClick={handleSend} disabled={!body.trim() || sendMsg.isPending} className="p-2.5 bg-cyan-600 hover:bg-cyan-700 disabled:opacity-50 text-white rounded-xl transition-colors">
            ...
            <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hidden md:flex flex-1 items-center justify-center flex-col gap-3 text-slate-400">
